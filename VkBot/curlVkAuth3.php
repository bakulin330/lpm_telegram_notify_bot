<?php
/*
Ќовый скрипт авторизации во ¬ онтакте на PHP с использованием CURL
 
(!) Ќе предусмотрен ввод каптчи
 
¬ы можете оптимизировать скрипт, избавив его от посто€нного запроса на авторизацию.
—ам не стал этим заниматьс€, так как лень ^.^
...
$login = '79163680325';
$password = '123456';
$security_check_code = '95195209'; // если требуетс€ 8 цифр номера телефона (по крайней мере у мен€ столько запросило)
...
// получаем ссылку дл€ редиректа после авторизации
preg_match('/Location\: (.*)/s', $post_auth['headers'], $post_auth_location);

if(!preg_match('/\_\_q\_hash=/s', $post_auth_location[1])) {
    echo 'Ќе удалось авторизоватьс€ <br /> <br />'.$post_auth['headers'];

    exit;
}

// переходим по полученной дл€ редиректа ссылке
...
if(preg_match('/act=security\_check/s', $get_my_page['headers'])) {
    ...
    // получаем hash дл€ запроса на проверку мобильного телефона
    ...
    echo '«апрошена проверка безопасности';
